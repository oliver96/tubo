...
abstract class Model {
    ...
    protected function parseWhere($key, $str) {
        $ops    = array('eq' => '=', 'gt' => '>', 'lt' => '<', 'in' => 'IN', 'notin' => 'NOT IN', 'like' => 'LIKE', 'not' => '!=', 'lte' => '<=', 'gte' => '>=');
        $pos    = strpos($str, ' ');
        ...
        if(property_exists($this, 'fields')) {
            ...
            if(!empty($fields)) {
                ...
                foreach($fields as $field) {
                    $field = trim($field);
                    if($field == $priKey) continue; // 主键不能修改
    ...
    public function truncate() {
        ...
        $affecteds  = 0;
        $affecteds  = $conn->execute(sprintf("TRUNCATE TABLE `%s`", $tableName));
        return $affecteds;
    }
